...
var versionCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		if !isSpringProject(".") {
			Error("Erreur: Lancez cette commande dans un projet Spring Boot (pom.xml ou build.gradle requis)")
			os.Exit(1)
		}

		cur, kind, err := detectVersionInBuildFiles(".")
		if err != nil {
			Error("failed to detect version: %v\n", err)
			os.Exit(1)
		}
		Info("Current version (%s): %s\n", kind, cur)
		...
		if versionSet != "" {
			...
			if err != nil {
				Error("failed to bump version: %v\n", err)
				os.Exit(1)
			}
			newVer = nv
		} else if versionAuto {
			...
			if err != nil {
				Error("failed to get last git tag: %v\n", err)
				os.Exit(1)
			}
			...
			if err != nil {
				Error("failed to bump auto version: %v\n", err)
				os.Exit(1)
			}
			newVer = nv
			Info("Auto new version (from %s): %s\n", last, newVer)
		...
		case "maven":
			if err := setVersionInPOM(".", newVer); err != nil {
				Error("failed to set version in pom.xml: %v\n", err)
				os.Exit(1)
			}
		case "gradle":
			if err := setVersionInGradle(".", newVer); err != nil {
				Error("failed to set version in build.gradle: %v\n", err)
				os.Exit(1)
			}
		default:
			Error("unsupported build file kind: %s\n", kind)
			os.Exit(1)
		}
		...
		if versionTag {
			...
			if err := exec.Command("git", "tag", tag).Run(); err != nil {
				Error("git tag failed: %v\n", err)
			} else {
				Success("Created tag %s\n", tag)
			}
		}
